                ...
                {filteredReports.length === 0 ? (
                  ...
                  filteredReports.map((report) => (
                    <div key={report.id} className="p-8 hover:bg-gray-50/50 transition-colors">
                      ...
                      {/* Review Section - Updated to handle array */}
                      {report.review && Array.isArray(report.review) && report.review.length > 0 && (
                        <div className="mb-8 p-6 bg-yellow-50 rounded-3xl border border-yellow-100">
                           <div className="flex justify-between items-center mb-2">
                              <div className="text-xs font-black text-yellow-600 uppercase">সর্বশেষ রিভিউ:</div>
                              <div className="text-xs font-bold text-yellow-600">মোট {report.review.length} টি</div>
                           </div>
                           <div className="flex items-center mb-2">
                              {[...Array(5)].map((_, i) => (
                                <svg key={i} className={`w-4 h-4 ${i < report.review![0].rating ? 'text-yellow-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                              ))}
                           </div>
                           <p className="text-gray-700 italic">"{report.review[0].comment}"</p>
                        </div>
                      )}
                      {/* Backward Compatibility for single object review */}
                      {report.review && !Array.isArray(report.review) && (
                        <div className="mb-8 p-6 bg-yellow-50 rounded-3xl border border-yellow-100">
                           <div className="text-xs font-black text-yellow-600 uppercase mb-2">রিভিউ:</div>
                           <div className="flex items-center mb-2">
                              {[...Array(5)].map((_, i) => (
                                <svg key={i} className={`w-4 h-4 ${i < (report.review as any).rating ? 'text-yellow-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                              ))}
                           </div>
                           <p className="text-gray-700 italic">"{(report.review as any).comment}"</p>
                        </div>
                      )}
